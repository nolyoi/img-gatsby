...
function Nav({ menuItem }) {
  ...
  const navLink = ({ to, title, description, bgColor, direction = "left" }) => {
    if (isMenuOpen && menuItem === title.toLowerCase().replace(/\s/g, "-")) {
      return <Link to={to} aria-label={title} title="Learn about our practice and team" className="font-bold tracking-wide text-blue-600 transition-colors duration-200 hover:text-deep-purple-accent-400">
        {title}
      </Link>
    } else if (isMenuOpen) {
                  ...
                  <div className="p-5 bg-white border rounded shadow-sm">
                    ...
                    <nav>
                      <ul className="space-y-4">
                        ...
                        <li>
                          <a href="https://login.patientfusion.com/?defaultTab=login&redirect=https%3A%2F%2Fmy.patientfusion.com" target="_blank" rel="noreferrer" className="inline-flex items-center justify-center w-full h-12 px-6 font-medium tracking-wide text-gray-700 transition duration-200 rounded shadow-md bg-blue-600 hover:bg-blue-800 focus:shadow-outline focus:outline-none" aria-label="Sign up" title="Sign up">
                            Patient Login
                          </a>
                        </li>
                      </ul>
